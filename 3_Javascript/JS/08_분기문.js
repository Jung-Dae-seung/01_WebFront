...
function startGame() {
  ...
  const number = Math.floor(Math.random() * 200) + 1; // 1~200 사이 난수 발생
  let inputNumber = prompt("번호를 입력하세요. (1~200)");
  let count = 0;                                      // 시도횟수
  ...
  while(true) {
    ...
    if(inputNumber > 200 || inputNumber < 1) {
      alert("범위를 벗어남");
      continue;
    }
    ...
    if(inputNumber > number) {
      inputNumber = prompt("[DOWN] 시도 횟수 : " + count);
    }

    // 입력된 값이 난수보다 낮을때
    if(inputNumber < number) {
      inputNumber = prompt("[UP] 시도 횟수 : " + count);
    }
